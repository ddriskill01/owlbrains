...
<?php 	
		...
		elseif ( $new_user != 'no' ) :
					if ( current_user_can( 'create_users' ) ){
						...
						$redirectLink = wp_redirect( home_url(‘/’) );
...
<?php			
				...
				if ( current_user_can( 'create_users' ) && $registration ) :
					$registerFilterArray['alertMessage1'] = '<p class="alert">'. __('', 'profilebuilder') .'</p><!-- .alert -->';
									...
									$registerFilterArray2['confirmationEmailForm'] = '
										<p class="send-confirmation-email">
											<label for="send-confirmation-email"> 
												...
												'. __('Send my Login Info via email.', 'profilebuilder') . '
											</label>
...
?>
						...
						<p class="form-submit">
							<input name="adduser" type="submit" id="addusersub" class="submit button" value="<?php if ( current_user_can( 'create_users' ) ) _e('Submit', 'profilebuilder'); else _e('Register', 'profilebuilder'); ?>" />
